...
## 0. 合并后方向纠偏：阅读收词，不是阅读器

> 记录日期：2026-07-09
> 状态：`lute-reading-mvp-design` 已合并进 `master` 后的产品收口决策。

阅读模块继续定位为 **阅读收词入口**，不是完整阅读器、专业阅读器或 Lute clone。

下一步小优化优先服务两件事：

- **去向感**：用户点击「加入学习」后，清楚知道词已进入本篇阅读的候选词，默认留在阅读器继续读，只提供轻量「去审核」入口。
- **来源感**：阅读候选在候选审核和词库详情里能看到轻量来源：`来自《文档名》` + PDF 原文例句。

本轮明确不做：

- 不增强成完整阅读器。
- 不做 EPUB / OCR / 章节 / 书签 / 阅读统计。
- 不做「回到原文位置」链接。
- 不把来源信息加到首页复习卡片。
- 不做单独的阅读候选审核页。

阅读候选继续复用现有候选审核页；仅对 `source_type='reading_pdf'` 的候选增加来源块。首页复习卡片保持轻量，暂不展示阅读来源。
