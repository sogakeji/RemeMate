...
Current active work is `feature/sessionpad-context-candidates-v1`: SP1 through SP4 and the whole-branch review fix
are complete and GCP-validated; the next step is an explicit merge/deploy decision. Read `HANDOFF.md` before the
Wayfinder documents because the latter preserve design decisions and are not the live Git status.
