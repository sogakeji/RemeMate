...
> 范围：在 GCP Ubuntu 独立测试机上，对 `feature/review-story-v1` @ `222d7c0` 补测并做 PostgreSQL 验收。  
...
| 项 | 结果 |
| --- | --- |
| 分支 / HEAD | `feature/review-story-v1` / `222d7c0` |
| 工作区（生产代码） | 干净；仅新增未提交测试文件 |
| migration | 两边升级到 **单一 head `f1a2b3c4d5e6`** |
| FORCE RLS | `review_story_runs`、`learning_funnel_events` 均 ENABLE+FORCE |
| RS1 定向 | **58 passed**（unit 33 + integration 25） |
| RLS 回归 | **19 passed** |
| 全量 `pytest -q` | **`539 passed, 16 warnings`，`FULL_RC=0`**（~29m） |
| `git diff --check` | 0 |
| `flask doctor --strict` | `DOCTOR_RC=1`：仅测试机 LLM/词典 WARN；DB/迁移/admin OK |
| 生产代码缺陷 | **未发现**（失败均来自测试夹具：RLS GUC 未注入、silent 日误断言 targets） |
| RS2 / AI / UI | **未做** |

**判定：RS1 数据地基 + 日内摘要 + 测试在独立 PostgreSQL 上行为可信。**  
...
| `tests/integration/test_review_stories.py` | 本地日/跨午夜、同词最差评分、跨用户/跨语言、eligibility 边界、2–5 目标与确定性、主释义、哈希、source/空词过滤、新表 FORCE RLS CRUD、唯一约束、并发冲突、事件类型白名单、check 约束、索引存在 |
...
- 未 commit / merge / push / 部署  

本地待提交（若授权）：

```text
?? tests/unit/test_review_stories.py
?? tests/integration/test_review_stories.py
?? docs/rs1-validation-2026-07-23.md
```
