...
- **下一项：Landing 移除过时的闭测与邀请制表述**（生产首次注册人测 2026-08-10）
  来源：生产开放注册已正式启用且真实邮箱注册、验证、自动登录与设密成功。当前
  `app/templates/main/landing.html` 仍显示 “Log in to closed beta”、"invitation-only"、
  “Already invited?” 和英文 footer 的 “A closed beta”，与实际准入状态冲突。
  下一轮作为独立低风险文案/UI 小票处理：开放态以注册为主要 CTA、登录为已有账号入口，重写邀请制区块和
  footer；关闭态仍须诚实隐藏注册入口。中英文、桌面与 390px 移动端以及开关两态一起验收，不改认证流程。
