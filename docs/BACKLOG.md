...
- **登录后应用 UI 分批国际化**（用户决策 2026-07-09）
  闭测门面先做公开 landing + 登录页中英切换，照顾中文学习者和外语语伴的进入门槛。
  登录后的完整应用暂不一次性全站 i18n，后续按核心路径分批：设置/首页复习/造句/词库/阅读收词。
  开工前需先定术语表和语言优先级，避免逐页硬翻译造成维护债。

- **移动端顶部栏布局：语言切换器和主题按钮应回到右上角**（用户反馈 2026-07-09）
  当前移动端顶部导航换行后，语言切换器和主题按钮被挤到下方，视觉上像页面内容的一部分。
  后续和移动端导航一起处理：品牌 logo/slogan 左侧，语言切换器 + 主题按钮右侧，与 logo 平行；
  主导航可另起一行或收进菜单，避免控件漂到卡片上方。
