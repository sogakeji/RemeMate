...
- **SessionPad：双人语言交换复盘信纸**（用户决策 2026-07-09）
  旧版 guest / WebSocket / room 实时协作方案已废弃。新的 SessionPad 定位为
  `我的 → 语言伙伴 → 复盘` 下的双人语言交换复盘信纸：一人一张信纸，分
  `帮自己记` / `帮他记` 两栏；帮自己记进入自己的候选词和词库，帮他记筛选后
  以快照反馈包推送给对方；对方可感谢、可加入自己的候选词审核。AI 只做会后总结
  （收获、讨论深度、主题词云、下次建议），不可阻塞保存/推送/入库。闭测期不立即实现，
  后续作为大功能单独开分支，详见 `docs/design/session-pad.md`。
...
- **SessionPad：邀请接受后建立双向伙伴关系**（闭测反馈 2026-07-11，P1）
  当前 A 创建 B 的资料并邀请 B 后，B 接受邀请只能完成账号绑定；A 不会自动出现在 B 的伙伴列表中。
  伙伴关系成立后，应引导 B 打开“确认新伙伴”页面。确认后，在 B 的伙伴列表中创建 A 的伙伴档案：
  昵称及双方语言方向从已有账号 / 邀请关系预填，私人备注仍由 B 自行维护。不得复制或暴露 A 对 B 的
  私人备注和历史复盘。语言方向的自动换位规则须在实现前用现有数据模型确认，不能把
  `feedback_language` 直接当作母语。

- **SessionPad：复盘发送状态与发送引导**（闭测反馈 2026-07-11，P1）
  语言伙伴页的复盘条目需要清楚区分“待发送”和“已发送”，并提供低干扰的发送入口或引导，让用户
  知道反馈应从哪里发给伙伴。状态必须来自真实反馈包 / 发送记录，不能仅由前端推断。

- **SessionPad：让发送者看见对方的感谢**（闭测反馈 2026-07-11，P1）
  收件人点击“感谢”后，发送者目前没有任何反馈。应让发送者在站内看到感谢结果，恢复最小但明确的
  互动闭环。实现前决定呈现位置和通知渠道；不引入“已读”“忽略”等带压力的状态，也不把感谢变成
  必须回应的社交任务。
...
- **SessionPad：反馈到候选词之间增加 AI 词语提取层**（闭测反馈 2026-07-11，P1）
  用户发送的反馈常是完整句子。当前直接加入候选词会生成句子级长卡片，违背候选审核和生词本的词语级
  心智模型。下一轮应在“收到反馈”与“生成候选词”之间增加提取步骤：AI 从用户勾选的反馈中拆出值得
  学习的词语 / 表达，候选卡保留原句作为来源语境。AI 不可用时必须允许手动编辑 / 拆分，不能阻塞查看
  反馈或造成内容丢失；提取结果仍需经过候选审核，不得直接入库。该项是现有 SessionPad B7“反馈加入
  候选词”能力的体验纠偏，不应通过增大候选输入框来掩盖数据粒度错误。
