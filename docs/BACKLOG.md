...
- **公开 Q&A / Blog 与 SEO 内容页**（用户决策 2026-08-14，获客）
  短计划见 `docs/plans/2026-08-14-public-seo-content.md`。范围已收口；SEO1 已完成开发并在新云机 staging 验证，当前
  尚未提交或合并入 `master`，正式环境未部署。静态双语、一页一语；无前缀英文、中文 `/zh`；Landing 只加随当前语言切换
  的入口；占位 200 但 `noindex`、不进 sitemap；首篇 slug 为 `why-word-lists-fail`。

- **SessionPad 易用性与智能辅助增强方向**（用户决策 2026-08-13，优先探索）
  团队扩容前不做微信小程序或 App Store 客户端，继续以网站为主，同时推进真实用户获取与现有功能增强。
  SessionPad 是当前最愿意继续增强的核心功能；现状的主要问题不是继续堆功能，而是录入、整理和发送反馈的
  UI 仍不够便利、智能，尚未形成让用户自然愿意持续使用的低摩擦体验。在“嘈杂环境下多人录音可可靠自动
  转录”达到可用水平前，优先探索无需依赖该能力的交互改进。具体方案尚未确定；启动前先结合真实使用观察，
  找出 SessionPad 各步骤的摩擦点和放弃点，明确一个可验证的高频场景、成功指标与最小改进切片，再进入设计
  和开发，避免预设为录音转录项目或一次性 UI 重做。
  > 过程记录（2026-08-13）：一次原本只针对 Landing 的 grilling 被擅自扩大到 SessionPad 数据模型、AI 整理、
  > beta 权限和运维设计，共追问 80 余项。该轮仅视为探索讨论，不构成规格、计划、待办或开发授权；不得据此
  > 派生其他开发任务。后续 grilling 必须守住用户原始范围，出现相邻产品问题时只记录边界并另行征求立项许可。
