...
- **导航信息架构：写一写聚合 + 语言伙伴独立入口**（用户决策 2026-07-11）
  等待用户指令后另开分支实现。顶部导航将「造句」改为「写一写」入口，在写作页内把
  「造句 / 历史 / 广场」作为同一工作域的同级视图，其中广场弱化到与历史同级；
  「语言伙伴」从「我的」菜单移出，成为独立一级入口。「收到的反馈」归属在实施前结合
  伙伴工作流确认。不得在 Landing/SessionPad 合并提交中夹带该改动。
