...
class UserQuota(db.Model):
    ...
    corrections_today  = db.Column(db.Integer, default=0, server_default=text("0"), nullable=False)  # 今日造句批改次数（按提交计）
    imports_today      = db.Column(db.Integer, default=0, server_default=text("0"), nullable=False)  # 今日导入候选词数（抽词/归一化时计）
